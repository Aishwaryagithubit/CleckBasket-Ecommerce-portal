...
if ($conn) {
    ...
    if ($shop) {
        $shopId = $shop['SHOP_ID'];
        $sql = "SELECT TO_CHAR(TRUNC(o.order_date), 'YYYY-MM-DD') AS order_date,
        ...
        oci_execute($stmt);
        while ($row = oci_fetch_assoc($stmt)) $dailyStats[] = $row;
...
render_trader_shell_start('Daily Reports', 'daily', 'TRADER PORTAL › REPORTS', 'Daily Reports', $right);
?>

<?php if (empty($dailyStats)): ?>
    <div class="card" style="padding:2rem;text-align:center;color:#888;">No daily data available yet.</div>
<?php else: ?>
<div class="report-grid">
    <div class="metric"><span>Total Orders</span>
        <strong><?= array_sum(array_column($dailyStats, 'TOTAL_ORDERS')) ?></strong></div>
    <div class="metric"><span>Total Sales</span>
        <strong>£<?= number_format(array_sum(array_column($dailyStats, 'TOTAL_SALES')), 2) ?></strong></div>
    <div class="metric"><span>Completed</span>
        <strong><?= array_sum(array_column($dailyStats, 'COMPLETED_ORDERS')) ?></strong></div>
</div>

<div class="card">
    <h2>Daily Orders (Last 7 Days)</h2>
    <table class="table">
        <tr><th>Date</th><th>Total Orders</th><th>Total Sales</th><th>Completed</th><th>Pending</th></tr>
